@extends('layouts.small-header')
@section('title'){{$title}}@endsection
@section('content')
    <!-- Contact -->
    <section class="about-info p-lg-5 p-3">
        <div class="content-w3ls-inn px-lg-5">
            <div class="container py-md-5 py-3">
                <div class="content-sing-w3pvt px-lg-5">
                    <h3 class="tittle-w3ls mb-3">Tesseral Pro</h3>
                    <p>Tesseral Technologies Inc. es una compañía internacional reconocida por desarrollar software
                        comercial que provee soluciones en el área de la Geofísica con el modelamiento de campo de onda
                        en un modelo digital de un cuerpo sólido y complejo. Además de otras soluciones que permiten al
                        usuario llevar a cabo gran variedad de interpretaciones pre-stack y post-stack. <br> Tesseral
                        Technologies Inc. es una compañía representada por Geo Oil Energy (GOE) y GOE Internacional Inc.
                        (Canadá), para la distribución y comercialización de sus productos y servicios en Latinoamérica.
                    </p>
                    <img class="img-fluid mb-2" src="/images/banner2.jpg" alt="">
                    <div class="row about-w3pvt-top mt-2">
                        <div class="col-lg-6 about-info">
                            <h4 class="title-hny mb-3">Características</h4>
                            <p>Es una aplicación que involucra un conjunto de soluciones de modelamiento sísmico de
                                onda completa para realizar evaluaciones geofísicas que simulen los efectos del
                                frente del campo de onda, como resultado de la propagación del campo de onda sísmico a través de modelos numéricos geológicos y geofísicos multi-parámetro, usando el método de Trazado de Rayos y Diferencia Finita, con el fin de brindar herramientas útiles y de alta precisión en el diseño de adquisición sísmica, y control de calidad del flujo de procesamiento e interpretación sísmica 2D y 3D.<br>
                                El trazado de Rayos y el método de diferencia finita son métodos de modelamiento numérico que permiten simular con precisión los efectos de onda durante la propagación del campo de onda sísmico en modelos geológicos multi-parámetro, donde la precisión del modelo estará limitada únicamente por la complejidad en la aproximación a la real ecuación de la onda.</p>
                        </div>
                        <div class="col-lg-6 about-info">
                            <h4 class="title-hny mb-3">Diseño de Adquisición Sísmica 2D & 3D</h4>
                            <p>La adquisición convencional, consiste en detectar ondas sísmicas a través de una línea
                                (sísmica 2D) o una grilla (sísmica 3D) de receptores ubicados cerca o sobre la
                                superficie de un área geológica de interés. Como resultado, se obtienen “Shotgathers”
                                para las fuentes disponibles y su correspondiente geometría observada, los cuales
                                conforman la información de adquisición. <br>
                                De forma similar, el modelamiento de onda completa con Tesseral, puede ser usado en diferentes etapas del diseño de adquisición sísmica para su control de calidad, generando “Shotgathers sintéticos” que pueden ser analizados de forma similar a los reales. (Interpretación a nivel Pre-Stack).</p>
                        </div>
                    </div>
                    <div class="row about-w3pvt-top mt-2">
                        <div class="col-lg-6 about-info">
                            <h4 class="title-hny mb-3">Procesamiento sísmico 2D & 3D</h4>
                            <p>En el flujo de procesamiento de información sísmica, se realiza la carga de información 2D y/o 3D (luego de algunas actualizaciones y mejoras previas al procesamiento), y se generan imágenes sísmicas que revelan límites geológicos e interfaces a diferentes tiempos y profundidades. En la planeación del flujo de procesamiento de información sísmica, el modelamiento de onda completa que brinda Tesseral, nos ayuda a evaluar la secuencia de procesamiento y parámetros apropiados a aplicar a la información sísmica de su compañía, haciendo control de calidad sobre los resultados que espera obtener. (Interpretación a nivel Post-Stack)</p>
                        </div>
                        <div class="col-lg-6 about-info">
                            <h4 class="title-hny mb-3">Herramientas de Análisis Visual – Trazado de Rayos e Imágenes Instantáneas</h4>
                            <p>Con el fin de analizar los sintéticos generados, Tesseral provee herramientas de análisis visual como el trazado de rayos de horizontes objetivo y películas de la propagación de onda, con el objetivo de identificar eventos que correspondan a la compresión o a las ondas convertidas reflejadas de los elementos objetivo de un sismograma sintético.</p>
                        </div>
                    </div>
                    <h3 class="title-hny my-2">Capacidades de Tesseral</h3>
                    <p>Con Tesseral puede generar “Gathers” sintéticos. Películas del campo de onda, tiempo de campo de las ondas incidentes, Secciones de tiempo y otras imágenes sísmica, relaciones AVO, entre otros para el análisis de modelos de complejidad arbitraria (con alto contraste de velocidad lateral, límites abruptos, compartimentalización, etc.), topografía compleja y condiciones cerca de la superficie (diferentes elevaciones de fuentes y receptores, trampas, permafrost, zonas de baja velocidad y espesor variable, refracciones y dinámica de ondas de superficie), modelos de capas delgadas construidos a partir de registros acústicos y densidad, con propiedades visco-elásticas, modelos de medios porosos saturados de fluidos. Como resultado, con Tesseral podrá dar recomendaciones para el diseño de adquisición sísmica, avalar el flujo de procesamiento sísmico para cualquier información sísmica (sísmica convencional, micro-sísmica, VSP o simulación de registros de pozo de ala frecuencia), modelamiento AVO para medios porosos, anisotrópicos, saturados de fluidos cambiando las propiedades físicas tanto en la lateral como en la vertical, verificar la veracidad de la interpretación de la interpretación sísmica de las discontinuidades reflejadas y demás elementos estructurales, entre otras.
                    </p>
                </div>
            </div>
        </div>
    </section>
    <!-- //single -->

@endsection